...
$GLOBALS['CODECITY'] = array(	 "HAN" => "Hà Nội",
    "HPH" => "Hải Phòng",
    "VDO" => "Vân Đồn",
    "DIN" => "Điện Biên",
    "VII" => "Vinh",
...
$GLOBALS['NATION'] = array(	 "HAN" => "Việt Nam",
    "HPH" => "Việt Nam",
    "VDO" => "Việt Nam",
    "DIN" => "Việt Nam",
    "VII" => "Việt Nam",
...
$GLOBALS['AIRPORT'] = array( "HAN" => "Nội Bài (HAN)",
    "HPH" => "Cát Bi (HPH)",
    "VDO" => "Vân Đồn (VDO)",
 ...
 */
function date_of_currentdate($selected_date, $way_flight = 1, $is_mobile = false) // date format: dd/mm/yyyy
{
    date_default_timezone_set('Asia/Ho_Chi_Minh');
    $dep_date = str_replace('/', '-', $selected_date);
    ...
    for ($i = -3; $i <= 3; $i++) {
        if ($i == 0)
            ...
            $show_date = date('Y-m-d', strtotime($dep_date . ' ' . ($i < 0 ? '-' . abs($i) : '+' . $i) . ' days'));

        $first_item = $i == -3 ? ' hidden-xs' : '';
        $active = strtotime($show_date) == strtotime($dep_date) ? ' active' : '';
        $disable = strtotime($show_date) < strtotime($curr_date) ? ' disable' : '';

        $show_date_txt = date('d/m/Y', strtotime($show_date));
        $show_date_mobile = date('d/m', strtotime($show_date));

        $html .= '<li class="date-item'. $first_item . $active . $disable . '">
            <a rel="' . $show_date_txt . '" class="'.($way_flight == 0 ? 'changedepartflight' : 'changereturnflight').'">
                <span>' . echoDate($show_date_txt, $is_mobile) . '</span>
                <span>' . ($is_mobile ? $show_date_mobile : $show_date_txt) . '</span>
            </a>
        </li>';
    }
 ...
 */
function echoDate($day, $is_mobile = false){ // $day format : dd/mm/yyyy
    $date = explode('/',$day);
    $dd   = $date[0];
    $mm   = $date[1];
    $yyyy = $date[2];
...
function Dep_addBaggage($airline, $ticketClass = ''){
    $str = '';
    if($airline == 'vietnamairline'){
        $str = '<tr><td>Xách tay</td><td>Mỗi người tối đa 12 Kg.</td></tr>
				<tr><td>Ký gửi</td><td>Mỗi người tối đa 23 Kg.</td></tr>';
    }
    elseif($airline == 'jetstar'){
        $str = '<tr><td>Xách tay</td><td>Mỗi người tối đa 7 Kg.</td></tr>
				<tr>
					<td>Ký gửi</td>
					<td>
						<select name="dep_addbaggage[]" class="form-control dep_addbaggage">
    ...
    elseif($airline == 'jetstar'){
        $str = '<tr><td>Xách tay</td><td>Mỗi người tối đa 7 Kg.</td></tr>
				<tr>
					<td>Ký gửi</td>
					<td>
						<select name="ret_addbaggage[]" class="form-control ret_addbaggage">
							<option value="0">Không mang hành lý ký gửi</option>
							<option value="220000">Thêm 15kg hành lý (220.000 VND/Khách)</option>
							<option value="240000">Thêm 20kg hành lý (240.000 VND/Khách)</option>
							<option value="330000">Thêm 25kg hành lý (330.000 VND/Khách)</option>
...
function checkactive($dep, $des, $return_list = false)
{
    $jstair = array();
    ...
    $vjair["DAD"] = array("SGN", "HAN", "VCA", "HPH", "NHA", "CXR");
    $vjair["NHA"] = array("SGN", "HAN", "HPH", "THD", "DAD", "VCA");
    $vjair["CXR"] = array("SGN", "HAN", "HPH", "THD", "DAD", "VCA");
    $vjair["HPH"] = array("SGN", "DAD", "NHA", "CXR", "PXU", "PQC", "DLI", "BMV", "VCA");
    $vjair["HUI"] = array("SGN", "HAN");
    $vjair["VII"] = array("SGN", "DLI", "PXU", "BMV", "VCA");
    $vjair["DLI"] = array("HAN", "VII", "SGN", "HPH", "VCA");
    $vjair["BMV"] = array("SGN", "HAN", "VII", "HPH");
    $vjair["UIH"] = array("SGN", "HAN");
    $vjair["VCA"] = array("DAD", "HAN", "HPH", "VII", "THD", "NHA", "CXR", "DLI");
    $vjair["PQC"] = array("SGN", "HAN", "HPH");
    $vjair["THD"] = array("SGN", "NHA", "CXR", "VCA");
    ...
    $qh = array();
    $qh["SGN"] = array("HAN", "UIH", "THD", "VII", "VDO", "HPH");
    $qh["HAN"] = array("SGN", "BMV", "DAD", "VDH", "UIH", "NHA", "CXR", "PQC", "VCA", "VII", "DLI", "PXU");
    $qh["HPH"] = array("SGN", "UIH", "VCA");
    $qh["DAD"] = array("HAN");
    $qh["BMV"] = array("HAN", "VII");
    $qh["VDH"] = array("HAN");
    $qh["CXR"] = array("HAN");
    $qh["NHA"] = array("HAN");
    $qh["PQC"] = array("HAN");
    $qh["VCA"] = array("HAN", "HPH");
    $qh["VII"] = array("HAN", "SGN", "BMV", "DLI");
    $qh["DLI"] = array("HAN", "VII");
    $qh["PXU"] = array("HAN");
    $qh["UIH"] = array("SGN", "HAN", "HPH");
    $qh["THD"] = array("SGN");
    $qh["VDO"] = array("SGN");

    if ($return_list) {
        $rs = array_merge(
            isset($jstair[$des]) ? $jstair[$des] : array(), 
            isset($vjair[$des]) ? $vjair[$des] : array(), 
            isset($vna[$des]) ? $vna[$des] : array(),
            isset($qh[$des]) ? $qh[$des] : array()
        );
